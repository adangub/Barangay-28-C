...
if(isset($_POST['rMonth']) && isset($_POST['rYear']) && isset($_POST['rType'])){
	...
	// Text
	$pdf->WriteTag(0,5,'<n>'.$type.' as of '.$monthtoget.' '.$yeartoget.'</n>',0,"C",0,0);
	$pdf->Ln(2);
	...
	{
		$selectSql=mysqli_query($conn,"SELECT * FROM transactions WHERE MONTH(date) = '$month' AND YEAR(date) = '$year'");
		$pdf->SetFont('times','',12);
		$pdf->SetWidths(array(25,45,40,60,17));
		$pdf->Row(array('Date','Resident','Employee','Request Type', 'Amount'));
		while($row = mysqli_fetch_array($selectSql))
		{
			$amount = $row['amount'];
			$date = $row['date'];
			...
			$res=mysqli_query($conn,"SELECT GET_FULLNAME(id,1) cname FROM resident WHERE id = '$resident_id'");
			$resrow = mysqli_fetch_array($res);
			$cname = $resrow['cname'];
			
			$emp=mysqli_query($conn,"SELECT position FROM employee WHERE id = '$employee_id'");
			$emprow = mysqli_fetch_array($emp);
			$position = $emprow['position'];
			
			$req=mysqli_query($conn,"SELECT request_name FROM request_type WHERE id = '$request_type_id'");
			$reqrow = mysqli_fetch_array($req);
			$request_name = $reqrow['request_name'];
			
			
			$transactiondate = date_create($row['date']);
			
			$transactionday = date_format($transactiondate, 'F j');
			
			$pdf->SetFont('times','',11);
			srand(microtime()*1000000);
			
			$pdf->Row(array($transactionday,$cname,$position,$request_name,$amount));
		}
	}
