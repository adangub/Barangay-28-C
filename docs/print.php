...
if(isset($_POST['requestID']) && isset($_POST['ctcno']) && isset($_POST['ctcdate']) && isset($_COOKIE['ux'])){
	if($_POST['reqType']=='Barangay Clearance Request' || $_POST['reqType']=='Police Clearance Request' || $_POST['reqType']=='NBI Clearance Request'){
		...
		$reqtype = mysql_real_escape_string($_POST['reqType']);
		
		$ctcdate = date_create($ctcdate);
		$ctcdate = date_format($ctcdate, 'F j, Y');
		...
		// Text
		$pdf->WriteTag(0,10,'<h1><b><u>'.$reqtype.'</u></b></h1>',0,"C",0,0);
		$pdf->Ln(8);
...
if(isset($_POST['hearingid']) && isset($_POST['blotterid']) && $_POST['transactionType']=='blotter'){
	$blotterid = mysqli_real_escape_string($conn, $_POST['blotterid']);
	$hearingid = mysqli_real_escape_string($conn, $_POST['hearingid']);
	
	$selectSql=mysqli_query($conn,"SELECT complaint, date, complainant_id from blotter where id = '$blotterid';");
		$row = mysqli_fetch_array($selectSql);
		$complaint = $row['complaint'];
		$date_ = $row['date'];
		$complainant_id = $row['complainant_id'];

	$selectSql=mysqli_query($conn,"SELECT first_name, middle_name, family_name from resident where id = '$complainant_id';");
		$row = mysqli_fetch_array($selectSql);
		$first_name = $row['first_name'];
		$middle_name = $row['middle_name'];
		$family_name = $row['family_name'];
	...
	$selectSql=mysqli_query($conn,"SELECT resident_id from respondents where blotter_id = '$blotterid';");
		$row = mysqli_fetch_array($selectSql);
		$resident_id = $row['resident_id'];
		
	$selectSql=mysqli_query($conn,"SELECT schedule_date, start_time, end_time FROM hearing_schedule where id = '$hearingid';");
		$row = mysqli_fetch_array($selectSql);
		$sDate = $row['schedule_date'];
		$sStart = $row['start_time'];
		$sEnd = $row['end_time'];
		$timestampStart = strtotime($sDate." ".$sStart);
		$timestampEnd = strtotime($sDate." ".$sEnd);
			$timestampStart = date('F d \a\t h:i A', $timestampStart);
	...
	while($whi = mysqli_fetch_array($selectSqli)){
		...
		$first_name = $row['first_name'];
			...
			array_push($names, $full_name_respondents);
	...
	if(count($names)>4){
			for($i=0; $i<count($names); $i+=2){
				if($names[$i+1]==' ')
				$pdf->WriteTag(0,1,'<p>'.$names[$i].'</p>',0,"J",0,0);
				else 
				$pdf->WriteTag(0,1,'<p>'.$names[$i].', '.$names[$i+1].'</p>',0,"J",0,0);
				$pdf->Ln(6);
			}
	}
	else{
		for($i=0; $i<count($names); $i++){
		$pdf->WriteTag(0,1,'<p><u>'.$names[$i].'</u></p>',0,"J",0,0);
		$pdf->Ln(6);
		}
	}
	...
	$pdf->WriteTag(0,0,'<n>DATE: <u>'.date_format($date, 'F d, Y').'</u></n>',0,"J",0,0);
	$pdf->Ln(10);
	...
	$pdf->Ln(1);
	$pdf->WriteTag(0,8,'<p>Please appear on <u>'.$timestampStart.'</u> for a dialogue in the name of Law.</p>',0,"J",0,0);
	$pdf->Ln(14);
