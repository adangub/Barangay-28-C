...
if(isset($_POST['rMonth']) && isset($_POST['rDay']) && isset($_POST['rYear']) && isset($_POST['pMonth']) && isset($_POST['pDay']) && isset($_POST['pYear']) && isset($_POST['rType'])){
	$month = mysqli_real_escape_string($conn, $_POST['rMonth']);
	$year = mysqli_real_escape_string($conn, $_POST['rYear']);
	$day = mysqli_real_escape_string($conn, $_POST['rDay']);
	
	$type = mysqli_real_escape_string($conn, $_POST['rType']);
	
	$mm = mysqli_real_escape_string($conn, $_POST['pMonth']);
	$yy = mysqli_real_escape_string($conn, $_POST['pYear']);
	$dd = mysqli_real_escape_string($conn, $_POST['pDay']);

	$datetogetfrom = $year.'-'.$month.'-'.$day;
	$datetogetto = $yy.'-'.$mm.'-'.$dd;
	
	$dateto = date_create($datetogetfrom);
	$datefrom = date_create($datetogetto);
	
	$toget = date_format($dateto, 'F j Y');
	$fromget = date_format($datefrom, 'F j Y');
	...
	// Text
	$pdf->WriteTag(0,5,'<n>'.$type.' Report as of '.$toget.' - '.$fromget.'</n>',0,"C",0,0);
	$pdf->Ln(2);
	...
	if($type == "Residency")
	{
		...
		$selectSql=mysqli_query($conn,"SELECT regdate, gender, birth_date, birth_place, precinctNo, civilstatus, email, contact_number, GET_FULLNAME(id,1) cname FROM resident WHERE regdate BETWEEN '$datetogetfrom' AND '$datetogetto';");
		$pdf->SetFont('times','',12);
		$pdf->SetWidths(array(30,45,23,19,16,21,26));
		$pdf->Row(array('Date','Full name','Birthday','Gender','Precinct','Status','Contact'));
		while($row = mysqli_fetch_array($selectSql))
		{
			...
			$pdf->Row(array($date,$fullname,$residencybirthdate,$gender,$precinctNo,$civilstatus,$contact_number));
		}
	
	}
	else if($type == "Barangay Clearance")
	{
		$selectSql=mysqli_query($conn,"SELECT GET_FULLNAME(transactions.resident_id, 0) cname, transactions.resident_id, requests.id, requests.optional_comment, request_type.request_name, requests.date 'request_date', requests.verified, requests.transactions_id, transactions.amount, transactions.date 'transactions_date', GET_EMPNAME(transactions.employee_id,0) 'empname' FROM transactions INNER JOIN requests ON requests.transactions_id=transactions.id  INNER JOIN request_type ON requests.request_type_id = request_type.id WHERE request_type.request_name = 'Barangay Clearance Request' AND transactions.date BETWEEN '$datetogetfrom' AND '$datetogetto';");
		$pdf->SetFont('times','',12);
		$pdf->SetWidths(array(25,45,40,30,17,20));
		$pdf->Row(array('Date', 'Request Type','Resident','Attended by', 'Amount', 'Status'));
		while($row = mysqli_fetch_array($selectSql))
		{
			...
			$pdf->Row(array($date, $request_name, $resident_name,$employee_name,$amount,$verified));
		}
	}
	else if($type == "Police Clearance")
	{
		$selectSql=mysqli_query($conn,"SELECT GET_FULLNAME(transactions.resident_id, 0) cname, transactions.resident_id, requests.id, requests.optional_comment, request_type.request_name, requests.date 'request_date', requests.verified, requests.transactions_id, transactions.amount, transactions.date 'transactions_date', GET_EMPNAME(transactions.employee_id,0) 'empname' FROM transactions INNER JOIN requests ON requests.transactions_id=transactions.id  INNER JOIN request_type ON requests.request_type_id = request_type.id WHERE request_type.request_name = 'Police Clearance Request' AND transactions.date BETWEEN '$datetogetfrom' AND '$datetogetto';");
		$pdf->SetFont('times','',12);
		$pdf->SetWidths(array(25,45,40,30,17,20));
		$pdf->Row(array('Date', 'Request Type','Resident','Attended by', 'Amount', 'Status'));
		while($row = mysqli_fetch_array($selectSql))
	...
	else if($type == "NBI Clearance")
	{
		$selectSql=mysqli_query($conn,"SELECT GET_FULLNAME(transactions.resident_id, 0) cname, transactions.resident_id, requests.id, requests.optional_comment, request_type.request_name, requests.date 'request_date', requests.verified, requests.transactions_id, transactions.amount, transactions.date 'transactions_date', GET_EMPNAME(transactions.employee_id,0) 'empname' FROM transactions INNER JOIN requests ON requests.transactions_id=transactions.id  INNER JOIN request_type ON requests.request_type_id = request_type.id WHERE request_type.request_name = 'NBI Clearance Request'  AND transactions.date BETWEEN '$datetogetfrom' AND '$datetogetto';");
		$pdf->SetFont('times','',12);
		$pdf->SetWidths(array(25,45,40,30,17,20));
		$pdf->Row(array('Date', 'Request Type','Resident','Attended by', 'Amount', 'Status'));
		while($row = mysqli_fetch_array($selectSql))
	...
	{
		$selectSql=mysqli_query($conn,"SELECT *, GET_COMPLAINANTNAME(complainant_id) cname FROM blotter WHERE date BETWEEN '$datetogetfrom' AND '$datetogetto';");
		$pdf->SetFont('times','',12);
		...
		while($row = mysqli_fetch_array($selectSql))
		{
			...
			$pdf->Row(array($blotterday,$complaint_name,$cname,$status));
